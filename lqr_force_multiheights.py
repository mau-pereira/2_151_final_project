...
# ... (all your symbolic EOM, compute_linearization_from_eom, ee_jacobian, etc.) ...

# -------------------------------------------------------------------
# SCENARIO CONFIGS (update q_desired to your actual ones)
# -------------------------------------------------------------------
HEIGHT_CONFIGS = [
    {
        "name": "low",
...
F_N_DES = 10           # desired normal force
...
def run_scenario(height_cfg):
    ...
    Returns a list of dicts (rows) to be written to CSV.
    """
    name = height_cfg["name"]
    q_desired = height_cfg["q_desired"]
    wall_site_name = height_cfg["wall_site_name"]
    ...
    joint_ids = [mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_JOINT, jn)
                 for jn in joint_names]
    qpos_addrs = [model.joint(jid).qposadr[0] for jid in joint_ids]
    qvel_addrs = [model.joint(jid).dofadr[0] for jid in joint_ids]
    actuator_ids = [mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_ACTUATOR, an)
    ...
    # ----- set equilibrium for linearization -----
    for i, addr in enumerate(qpos_addrs):
        data.qpos[addr] = q_desired[i]
    ...
    # contact direction / stiffness for C_F
    ...
    for step in range(n_steps):
        ...
        # joint error
        e_q = x_joint[:3] - x_desired_joint[:3]
        joint_err_norm = np.linalg.norm(e_q)

        # EE position and errors
        ee_pos = data.site_xpos[ee_site_id].copy()
        ee_err_vec = ee_pos - x_ee_eq
        ee_err_norm = np.linalg.norm(ee_err_vec)

        x_n_curr = float(np.dot(ee_pos - x_wall_eq, n_hat))
        x_n_eq   = float(np.dot(x_ee_eq - x_wall_eq, n_hat))
        x_n_err  = x_n_curr - x_n_eq
...
# -------------------------------------------------------------------
# MAIN: run all three scenarios and write one CSV
# -------------------------------------------------------------------
all_rows = []
